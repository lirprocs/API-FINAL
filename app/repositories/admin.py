...
async def create_update_submenu(submenu: SubMenuType, manual_id: str):
    async with async_session_maker() as session:
        ...
        parent_id = menu_id_result.scalar_one_or_none()
        ...
        if db_submenu:
            ...
            new_submenu = SubMenu(title=submenu[2], description=submenu[3], parent_id=parent_id, manual_id=manual_id)
...
async def create_update_dish(dish: DishType, manual_id: str):
    async with async_session_maker() as session:
        ...
        parent_id = submenu_id_result.scalar_one_or_none()
        ...
        if db_dish:
            ...
            new_dish = Dish(title=dish[3], description=dish[4], price=dish[5],
                            parent_id=parent_id, manual_id=manual_id)
